...
fn main() {
    // ROOT_OUT_DIR is set by the top-level octox build.rs when it invokes
    // `cargo install uprogs` to populate the fs.img. When uprogs is built
    // directly as a workspace member (e.g. `cargo build --target ...` at
    // the repo root), the variable is absent — skip the copy step then.
